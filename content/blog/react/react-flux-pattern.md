...
![view](./images/flux_09.JPG)

뷰는 발표자와 같다

앱 내부에 대해서는 아는 것이 없지만 받은 데이터를 잘 처리해서 사람들이 이해할 수 있는 format(HTML)으로 어떻게 바꾸는지 알고 있다

## Controller view

컨트롤러 뷰는 스토어와 뷰 사이의 중간관리자 역할을 한다

상태가 변경되었을 때 스토어가 그 사실을 컨트롤러 뷰에게 알려주면 컨트롤러 뷰는 자신의 아래에 있는 모든 뷰에게 새로운 상태를 넘겨준다

# 어떻게 함께 동작할까?

이제 어떻게 `Action`, `Dispatcher`, `Store`, `Controller view and View`가 함께 동작하는지 알아보도록 하자

## 준비(Setup)

준비과정은 앱이 초기화할 때 딱 한 번 거치는 과정이다

![setup_01](./images/flux_10.JPG)

1. 스토어는 디스패처에 액션이 들어오면 알려달라고 말해둔다

2. 컨트롤러 뷰는 스토어에게 최신 상태를 묻는다

![setup_03](./images/flux_11.JPG)

3. 스토어가 컨트롤러 뷰에게 상태를 주면 렌더링하기 위해 모든 자식 뷰에게 상태를 넘겨준다

![setup_04](./images/flux_12.JPG)

4. 컨트롤러 뷰는 스토어에게 상태가 바뀔 때 알려달라고 다시 부탁한다

## 데이터의 흐름(Data flow)

![data flow](./images/flux_13.JPG)

준비 과정이 끝나면 앱은 유저들의 입력 위한 준비가 완료된다

유저의 입력으로 인한 액션이 생긴다면 다음과 같은 데이터 흐름을 만든다

![data flow_01](./images/flux_14.JPG)

1. 뷰는 액션 생성자에게 액션을 준비하라고 말한다

![data flow_02](./images/flux_15.JPG)

2. 액션 생성자는 액션을 포맷에 맞게 만들어 디스패처에 넘겨준다

![data flow_03](./images/flux_16.JPG)

3. 디스패처가 들어온 액션의 순서에 따라 알맞은 스토어에 보내면, 각 스토어는 모든 액션을 받게 되지만 필요한 액션만을 골라 상태를 변경한다

![data flow_04](./images/flux_17.JPG)

4. 상태 변경이 완료되면 스토어는 자신을 구독(subscribe)하고 있는 컨트롤러 뷰에게 그 사실을 알린다

5. 연락을 받은 컨트롤러 뷰들은 스토어에게 변경된 상태를 요청한다

![data flow_06](./images/flux_18.JPG)
